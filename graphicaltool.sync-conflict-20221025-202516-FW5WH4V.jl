...
using PlutoUI, DataFrames, XLSX, CSV, Plots
...
for file in files
    ...
    if file[end-2:end] == "lsx" && file[1]!="~"
		try
        push!(frames, DataFrame(XLSX.readtable(path, 1,
			header = false, first_row = 2)))
        push!(labels, file[1:end-5])
		catch err
			println(err)
		end
	elseif file[end-2:end] == "csv"
		push!(frames, DataFrame(CSV.read(path, DataFrame; header=false, skipto=3)))
		push!(labels, file[1:end-4])
    end
end
end
...
# ╔═╡ 4b400617-229e-4cf7-9093-44966c8a8c22
begin
plot(ydata, legend=:bottomright, label="data")
ylims!((max(ydata...)/2, max(ydata...)))
vline!([a], label = "a")
vline!([b], label = "b")
vline!([c], label = "c")
vline!([d], label = "d")
...
begin
	...
	df1 = frame[range1, :]
	df2 = frame[range2, :]
	plot(frame[:, 2], label = "Data", legend = :bottomright)
	plot!((1:length(df1[:, 2])) .+ a, df1[:, 2], fill =(0, 0.1, :red), 
	label = "mean 1: $(mean(df1[:,2]))")
	plot!((1:length(df2[:, 2])) .+ c, df2[:, 2], fill = (0, 0.1, :green), 
	label = "mean 2: $(mean(df2[:,2]))")
end

# ╔═╡ bda066ee-a141-4cf5-b6e7-a4b031c8bcb8
html"""
<style>
input[type*="range"] {
	width: 100%;
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "92f172d7928b330adb7dd7a0266b116aeacafba0"
